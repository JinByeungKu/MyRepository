...
public class PhotoBoardDao {
	...
	public List<PhotoBoard> selectByPage(int pageNo, int rowsPerPage) {
		String sql = "";
		sql += "select rn, bno, btitle, bcontent, savedfile ";
		sql += "from ( ";
		sql += "select rownum as rn, bno, btitle, bcontent, savedfile ";
		sql += "from (select bno, btitle, bcontent, savedfile from photoboard order by bno desc) ";
		sql += "where rownum<=? ";
		sql += ") ";
		sql += "where rn>=? ";
